...
- Use absolute `$WS` paths.
- Follow [release-pr](../skills/release-pr.md) for CI, review waves, tracker writes, and caps. **Never green a check by weakening it** — no lowered coverage threshold, no `skip`/`xfail`, no disabled lint rule. A red check is routed, never patched here.
