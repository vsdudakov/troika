...
Run [release-notes.md](../release-notes/SKILL.md) for this release, diffing the previous release's **branch head** against the new branch — not its tag. Open its PR, get it merged so the notes land on the default branch, and keep its per-item QA blocks: step 4 is built from them.
