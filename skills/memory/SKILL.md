...
when, and what it cost. The directory is `$TROIKA_MEMORY` (workspace paths (`#workspace-paths`)).
...
| --- | --- |
| the profile, `$TROIKA_PROFILE` | facts about this organisation: repos, commands, stack, tracker, voice |
...
- `promoted` — moved to its permanent home: the workspace profile when it is an
